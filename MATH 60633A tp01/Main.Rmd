...
The VaR forecast and the log-returns for the S&P 500 index are plotted, and the graph is saved as a `png` file in a new folder created called `Output`.
...
# Load the results of the backtest
```{r rda, echo=TRUE}
rda=get(load(here("Outputs/backtest_results.rda")))
rda
```

# plot rda
```{r rda-plot, echo=TRUE}
plot(rda$SP500, type = 'l', col = 'red', xlab = "Window", ylab = "Returns",
     main = "Backtest Results for S&P 500", ylim = c(-0.1,0.1), xlim = c(0,1000))
```
