...
xts_conditional_variances <-
  xts(VaR_forecast_sp500$ConditionalVariances,
      order.by = as.Date(dates_vector) + 1)

# Plot the conditional variances as a time series
plot(
  sqrt(xts_conditional_variances),
  type = "l",
  xlab = "Garch Standard Deviation",
  main = "Garch Standard Deviation by date",
  col = "blue"
)
...
cat(
  "VaR and Params from implemented garch:\nVaR:",
...
hist(
  log_returns$SP500,
  main = "SP500 daily log-returns",
  col = "blue",
  breaks = 100
)
abline(v = rugarch_test$VaR,
       col = "blue",
       lwd = 4)
abline(v = VaR_forecast_sp500$VaR_Forecast,
       col = "red",
       lwd = 2)
abline(
  v = quantile(log_returns$SP500, 0.05),
  col = "black",
  lwd = 2
)
legend(
  "topright",
  legend = c("VaR from RuGarch", "VaR from Implemented Garch", "VaR from data"),
...
cat("VaR at 95% for FSTE100:",
    VaR_forecast_fste100$VaR_Forecast,
    "\n")

most_risky = ifelse(
  VaR_forecast_sp500$VaR_Forecast < VaR_forecast_fste100$VaR_Forecast,
  "SP500",
  "FSTE100"
)
...
cl <-
  makeCluster(detectCores() - 2) # Leave 2 cores free for system processes
clusterExport(
  cl,
  list(
    "f_forecast_var",
...
dates <-
  index(log_returns_sp500)[window_size:(window_size + number_of_windows - 1)]
VaR_forecast_sp500_xts <-
  xts(VaR_forecast_sp500_vector, order.by = dates)
...
returns = log_returns_sp500[window_size:(window_size + number_of_windows -
                                           1)]
daysBelowVaR = sum(returns < VaR_forecast_sp500_vector)
percentageBelowVaR = daysBelowVaR / number_of_windows * 100
cat(
  "The percentage of time that the returns are below the VaR forecast is:",
  percentageBelowVaR,
  "\n"
)
