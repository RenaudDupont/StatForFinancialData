...
f_generate_datas <- function(theta, n)
{
  ...
  beta <- theta[3]
  epsilon <-
    rnorm(n)  # Random shocks from a standard normal distribution
...
hist(data,
     main = "data daily log-returns",
     col = "blue",
     breaks = 100)
abline(v = VaR_forecast_sp500$VaR_Forecast,
       col = "red",
       lwd = 2)
abline(v = quantile(data, 0.05),
       col = "black",
       lwd = 2)
legend(
  "topright",
  legend = c("VaR from Garch", "VaR from data"),
  col = c("red", "black"),
  lty = 1:1,
  cex = 0.8
)
...
mc_param = c(sum(params[, 1]) / n_sim, sum(params[, 2]) / n_sim, sum(params[, 3]) /
               n_sim)
mc_param